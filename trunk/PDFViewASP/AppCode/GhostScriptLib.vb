...
#Region "Convert"
    ...
    Private Function Convert(ByVal inputFile As String, ByVal outputFile As String, ByVal throwException As Boolean, ByVal options As String) As Boolean
      ...
      If String.IsNullOrEmpty(inputFile) Then
        ...
        Else
          Console.WriteLine("The inputfile is missing")
          Return False
        End If
      End If
      If Not System.IO.File.Exists(inputFile) Then
        ...
        Else
          Console.WriteLine(String.Format("The file :'{0}' doesn't exist", inputFile))
          Return False
        End If
      End If
      If String.IsNullOrEmpty(_sDeviceFormat) Then
        ...
        Else
          Console.WriteLine("You didn't provide a device for the conversion")
          Return False
        End If
      End If
      ...
      Try
        ...
        If intReturn < 0 Then
          ...
          If throwException Then
            Throw New ApplicationException("I can't create a new instance of Ghostscript please verify no other istance are running!")
          Else
            Console.WriteLine("I can't create a new instance of Ghostscript please verify no other istance are running!")
            Return False
          End If
        End If
      Catch ex As DllNotFoundException
        ...
        ClearParameters(aGCHandle, gchandleArgs)
        If throwException Then
          Throw New ApplicationException("The gsdll32.dll wasn't found in default DLL search path" & "or is not in correct version (doesn't expose the required methods). Please download " & "version 8.64 from the original website")
        Else
          'Barbara post write much better then me, thanks her for the nice words :P
          Console.WriteLine("The gsdll32.dll wasn't found in default DLL search path" & "or is not in correct version (doesn't expose the required methods). Please download " & "version 8.64 from the original website")
          System.Windows.Forms.MessageBox.Show("The gsdll32.dll wasn't found in default DLL search path" & "or is not in correct version (doesn't expose the required methods). Please download " & "version 8.64 from the original website")
          Return False
        End If
      End Try
...
#Region "Accessory Functions"
    ...
    End Function
#End Region
#Region "Manage Standard Input & Standard Output"
...
#Region "Simple Conversion Functions"
    ...
    Public Shared Function GetPageFromPDF(ByVal filename As String, ByVal destPath As String, ByVal PageNumber As Integer, Optional ByVal DPI As Integer = VIEW_DPI, Optional ByVal Password As String = "") As String
      Dim converter As New ConvertPDF.PDFConvert
      Dim Converted As Boolean = False
      converter.RenderingThreads = Environment.ProcessorCount
      converter.OutputToMultipleFile = False
      ...
      End If
      'Turn on anti-aliasing (smooth edges)
      converter.TextAlphaBit = 4
      converter.GraphicsAlphaBit = 4
